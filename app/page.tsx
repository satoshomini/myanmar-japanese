      ...
      <div className="px-4 pt-8 pb-3">
        ...
        <div className="flex gap-2 mb-4">
          {levelOrder.map(lvl => {
            ...
            const count = byLevel[lvl]?.length || 0;
            return (
              <div key={lvl} className="flex-1 bg-gray-900 rounded-xl px-3 py-2 text-center border border-gray-800">
                <div className={`text-lg font-bold ${cfg.color}`}>{count}</div>
                <div className="text-gray-500 text-[10px]">{cfg.emoji} {cfg.label}</div>
              </div>
            );
          })}
