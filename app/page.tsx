      ...
      <div className="px-4 pt-8 pb-3">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-gray-900 border border-gray-700 rounded-xl pl-9 pr-10 py-2.5 text-sm text-white placeholder-gray-600 focus:outline-none focus:border-gray-500"
          />
          {search && (
            <button onClick={() => { setSearch(""); setActiveTab("home"); }} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 text-lg leading-none">×</button>
          )}
        </div>
      </div>
    ...
    <Link href={`/lesson/${lesson.id}`} className={grid ? "block" : "flex-shrink-0 w-36"}>
      <div className="rounded-xl overflow-hidden bg-gray-900 border border-gray-800 active:scale-95 transition-all duration-150 hover:border-gray-600">
        <div className="relative aspect-video w-full bg-gray-800">
          ...
          <button onClick={(e) => onToggle(lesson.id, e)} className="absolute bottom-1.5 right-1.5 text-lg leading-none drop-shadow-lg active:scale-125 transition-transform">
            {isFav ? "❤️" : "🤍"}
          </button>
        </div>
        <div className="p-2">
